...
#elif  defined(__lynx__) || defined(__sun__) || defined(_AIX) || defined(__FreeBSD__) || defined(__NTVIS__) || defined(__NetBSD__) || defined(__CYGWIN__)
...
int bufsleep = 0;                      /* sleep in between buffers */
...
#ifdef BUILD_STATIC
int epittcp_main(int argc, char* argv[]); 
#else
...
#ifdef BUILD_STATIC
int
epittcp_main(int argc, char* argv[])
#else
int
...
#endif
{
	...
	if (sockbufsize) {
		...
		*/
		...
		getsockopt(fd, SOL_SOCKET, SO_RCVBUF, (char *)&sockbufrcvsize, 
		           &sockbuflen);
#else
		getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &sockbufsndsize, (socklen_t *)&sockbuflen);
		sockbuflen = sizeof sockbufrcvsize;
		getsockopt(fd, SOL_SOCKET, SO_RCVBUF, &sockbufrcvsize, (socklen_t *)&sockbuflen);
#endif /* __lynx__ __sun__ __NTVIS__ */
	...
	if (!udp) {
		...
		if (trans) {/* We are the client if transmitting */
			...
			if((fd=accept(fd, SOCKADDR_CAST &frominet, (socklen_t *)&fromlen) ) < 0) 
				sockets_err("accept");

			if (getpeername(fd, SOCKADDR_CAST &peer, (socklen_t *)&peerlen) < 0)
...
Nread(SOCKET s, void* bufp, int count )
{
	...
	if( udp ) {
		cnt = recvfrom( s, bufp, count, 0, SOCKADDR_CAST &from, (socklen_t *)&len );
		numCalls++;
	} else {
		if( b_flag )
