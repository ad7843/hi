...
void wlmngr_wlIfcUp(int idx)
{
   ...
   for ( i=0; i<m_instance_wl[idx].numBss; i++) {
      if (m_instance_wl[idx].m_wlMssidVar[i].wlEnblSsid) {
         ...
         sprintf(cmd, "ifconfig %s up", m_instance_wl[idx].m_ifcName[i]);
         bcmSystem(cmd);
      } else {
         sprintf(cmd, "ifconfig %s down", m_instance_wl[idx].m_ifcName[i]);
         bcmSystem(cmd);      	
      }
   }
}
...
void wlmngr_wlIfcDown(int idx)
{
   ...
   int i;
   for (i=0; i<m_instance_wl[idx].numBss; i++) {
         sprintf(cmd, "ifconfig %s down 2>/dev/null", m_instance_wl[idx].m_ifcName[i]);
         bcmSystem(cmd);
   }
}
...
void wlmngr_setupBridge(int idx)
{
   ...
   char cmd[WL_LG_SIZE_MAX];
      ...
      for (i=0; i<m_instance_wl[idx].numBss; i++) {
            sprintf(cmd, "brctl addif %s %s", m_instance_wl[idx].m_wlMssidVar[0].wlBrName, m_instance_wl[idx].m_ifcName[i]);      
            bcmSystem(cmd);
      }
...
void wlmngr_stopBridge(int idx)
{
   ...
   char cmd[WL_LG_SIZE_MAX];
      ...
      for (i=0; i < m_instance_wl[idx].numBss; i++) {
            sprintf(cmd, "brctl delif %s %s", m_instance_wl[idx].m_wlMssidVar[i].wlBrName, m_instance_wl[idx].m_ifcName[i]);      
            bcmSystem(cmd);
      }
 ...
 void wlmngr_getPwrSaveStatus(int idx, char *varValue)
{
    ...
    if ( fp != NULL ) { 
       if (rxchain == 1)
       {	
          sprintf(buf, "wlctl -i %s rxchain_pwrsave >/var/wlpwrsave1",  m_instance_wl[idx].m_ifcName[0]);
          BCMWL_WLCTL_CMD(buf);
          curLine = ftell(fp);
       ...
       { /* erase the line */
          ...
          sprintf(buf, "wlctl -i %s radio_pwrsave >/var/wlpwrsave1",  m_instance_wl[idx].m_ifcName[0]);
          BCMWL_WLCTL_CMD(buf);
          curLine = ftell(fp);
          fgets(buf, sizeof(buf), fp);
          radiopwr = atoi(buf);
...
void wlmngr_doSecurity(int idx) {
    ...
    sprintf(name, "%s", m_instance_wl[idx].m_ifcName[0]);
   ...
   for(i=0; i<m_instance_wl[idx].numBss; i++) {         	
      for ( j = 0; j < 4; j++ ) {
         sprintf(cmd, "wlctl -i %s rmwep -C %d %d",m_instance_wl[idx].m_ifcName[0],  i, j);
         BCMWL_WLCTL_CMD(cmd);
      }   
   }
...
void wlmngr_doSecurityOne(int idx, int ssid_idx) {
   ...
   sprintf(cmd, "wlctl -i %s  wsec -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, val);
   BCMWL_WLCTL_CMD(cmd);
   ...
   if ( ((strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_OPEN) == 0) ||
      ...
      val = 1;
   sprintf(cmd, "wlctl -i %s wsec_restrict -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, val);
   BCMWL_WLCTL_CMD(cmd);

   /* Set WPA authentication mode - radius/wpa/psk */  
   if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_RADIUS) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WPA_AUTH_DISABLED);
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WPA) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WPA_AUTH_UNSPECIFIED);
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WPA_PSK) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WPA_AUTH_PSK);
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WPA2) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WPA2_AUTH_UNSPECIFIED);
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WPA2_PSK) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WPA2_AUTH_PSK);      
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WPA2_MIX) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WPA_AUTH_UNSPECIFIED+WL_WPA2_AUTH_UNSPECIFIED);
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WPA2_PSK_MIX) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WPA_AUTH_PSK+WL_WPA2_AUTH_PSK);            
#ifdef BCMWAPI_WAI
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WAPI) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WAPI_AUTH);
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WAPI_PSK) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WAPI_AUTH_PSK);      
   else if (strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuthMode, WL_AUTH_WAPI_MIX) == 0)
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, (WL_WAPI_AUTH+WL_WAPI_AUTH_PSK));
#endif
   else
      sprintf(cmd, "wlctl -i %s wpa_auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, WL_WPA_AUTH_DISABLED);
   ...
   /* Set EAP restrict if WPA/radius is enabled */
      ...
      sprintf(cmd, "wlctl -i %s eap 1", m_instance_wl[idx].m_ifcName[0]);
   else
      sprintf(cmd, "wlctl -i %s  eap 0", m_instance_wl[idx].m_ifcName[0]);
   BCMWL_WLCTL_CMD(cmd);
   }
   ...
   if ( strcmp(m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlWep, WL_ENABLED) == 0 ) {
      if ( m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlKeyBit == WL_BIT_KEY_64 ) {
         ...
         for ( i = 0; i < 4; i++ ) {
            ...
            sprintf(cmd, "wlctl -i %s addwep -C %d %d %s", m_instance_wl[idx].m_ifcName[0], ssid_idx, i, key);
            BCMWL_WLCTL_CMD(cmd);
         }
         // add active key
         for ( i = 0; i < 4; i++ ) {
   ...
   sprintf(cmd, "wlctl -i %s auth -C %d %d", m_instance_wl[idx].m_ifcName[0], ssid_idx, m_instance_wl[idx].m_wlMssidVar[ssid_idx].wlAuth);
   BCMWL_WLCTL_CMD(cmd);
	...
	*cmd = '\0';
	if(!ifcName)
	{
		...
		list_for_each(pos, (m_instance_wl[idx].m_wlMssidVar[MAIN_BSS_IDX].m_tblFltMac) )  {
			sprintf(cmd, "wlctl %s %s", mode, pos->macAddress);
			BCMWL_WLCTL_CMD(cmd);
		}
	...
	else 
	{
		...
		list_for_each(pos, (m_instance_wl[idx].m_wlMssidVar[bssIdx].m_tblFltMac) )  {
			...
			if(!strcmp(ifcName, pos->ifcName)) {
				sprintf(cmd, "wlctl -i %s %s %s", ifcName, mode, pos->macAddress);
				BCMWL_WLCTL_CMD(cmd);
			}
		}
	}
...
void wlmngr_doMacFilter(int idx) {
   ...
   if(!m_instance_wl[idx].mbssSupported) {
	 ...
	 sprintf(cmd, "wlctl -i %s mac none", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
         sprintf(cmd, "wlctl -i %s macmode 2", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
	  ...
	  wlmngr_macFltSetup( "mac", NULL, idx ,0);	
         ...
         sprintf(cmd, "wlctl -i %s mac none", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
         sprintf(cmd, "wlctl -i %s macmode 1", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
         // add new MAC filter list
         wlmngr_macFltSetup( "mac", NULL, idx ,0);        
      } else {
	 ...
	 sprintf(cmd, "wlctl -i %s mac none", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
         sprintf(cmd, "wlctl -i %s macmode 0", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
      }
   } else {       
       for (i=0; i<m_instance_wl[idx].numBss; i++) {
             ...
             if ( strcmp(m_instance_wl[idx].m_wlMssidVar[i].wlFltMacMode, WL_FLT_MAC_ALLOW) == 0 ) {
                // clear all MAC filter lists
                sprintf(cmd, "wlctl -i %s mac none", m_instance_wl[idx].m_ifcName[i]);
                BCMWL_WLCTL_CMD(cmd);
                sprintf(cmd, "wlctl -i %s macmode 2", m_instance_wl[idx].m_ifcName[i]);
                BCMWL_WLCTL_CMD(cmd);            
                // add new MAC filter list
               wlmngr_macFltSetup( "mac", m_instance_wl[idx].m_ifcName[i], idx,i );             
                ...
                sprintf(cmd, "wlctl -i %s mac none", m_instance_wl[idx].m_ifcName[i]);
                BCMWL_WLCTL_CMD(cmd);
                sprintf(cmd, "wlctl -i %s macmode 1", m_instance_wl[idx].m_ifcName[i]);
                BCMWL_WLCTL_CMD(cmd);        
                // add new MAC filter list
               wlmngr_macFltSetup( "mac", m_instance_wl[idx].m_ifcName[i], idx ,i);            
             } else {
                ...
                sprintf(cmd, "wlctl -i %s mac none", m_instance_wl[idx].m_ifcName[i]);
                BCMWL_WLCTL_CMD(cmd);
                sprintf(cmd, "wlctl -i %s macmode 0", m_instance_wl[idx].m_ifcName[i]);
                BCMWL_WLCTL_CMD(cmd);
             }
          }
       }
         ...
         if( j >= WL_WDS_NUM)  { //new entry
            ...
            for (j = 0; j < WL_WDS_NUM; j++) {
               if(strlen(m_instance_wl[idx].m_wlVar.wlWds[j]))
                  ...
                  strcpy(m_instance_wl[idx].m_wlVar.wlWds[j],mac);
                  break;
               }                
            }        
         }        
...
void wlmngr_doWds(int idx) {
  ...
  /* get the MAC list */
   ...
   sprintf(cmd, "wlctl -i %s lazywds %d", m_instance_wl[idx].m_ifcName[0], enable);
   BCMWL_WLCTL_CMD(cmd);

      sprintf(cmd, "wlctl -i %s wds %s", m_instance_wl[idx].m_ifcName[0], maclist);
      BCMWL_WLCTL_CMD(cmd);
	  
   sprintf(cmd, "wlctl -i %s wdstimeout 1", m_instance_wl[idx].m_ifcName[0]);
   BCMWL_WLCTL_CMD(cmd);
}
#endif /* WLCONF */
...
void wlmngr_doWdsSec(int idx) {
   ...
   sprintf(cmd, "wlctl -i %s wdswsec 0", m_instance_wl[idx].m_ifcName[0]);
   BCMWL_WLCTL_CMD(cmd);           
   sprintf(cmd, "wlctl -i %s wdswsec_enable 0", m_instance_wl[idx].m_ifcName[0]);
   BCMWL_WLCTL_CMD(cmd);     
      ...
      for ( i = 0; i < WL_WDS_NUM; i++ ) {
         ...
         if(m_instance_wl[idx].m_wlVar.wlWdsKey[0] != '\0') {
            ...
            //goes to index 0/key 1
      	    sprintf(cmd, "wlctl -i %s addwep %d %s %s", m_instance_wl[idx].m_ifcName[0], 0, key, m_instance_wl[idx].m_wlVar.wlWds[i]);
            BCMWL_WLCTL_CMD(cmd);
            sprintf(cmd, "wlctl -i %s wdswsec 1", m_instance_wl[idx].m_ifcName[0]);
            BCMWL_WLCTL_CMD(cmd);
         }
      	 sprintf(cmd, "wlctl -i %s wdswsec_enable 1", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
      }
   } else {
      for ( i = 0; i < WL_WDS_NUM; i++ ) {   	
         ...
         //remove key index 0 for the mac
      	 sprintf(cmd, "wlctl -i %s rmwep %d %s", m_instance_wl[idx].m_ifcName[0], 0, m_instance_wl[idx].m_wlVar.wlWds[i]);
         BCMWL_WLCTL_CMD(cmd);    
      }   	
   }
}
...
void wlmngr_scanWdsStart(int idx) {
   ...
   //sprintf(cmd, "wlctl ssid ''");
   //bcmSystem(cmd);

   //sprintf(cmd, "wlctl down");
   //bcmSystem(cmd);

   //sprintf(cmd, "wlctl up");
   //bcmSystem(cmd);
}
    ...
    for (i = 0; i < list->count; i++) {
        ...
        if (bi->ie_length) {
            if(ap_count < MAX_AP_SCAN_LIST_LEN){
                ...
                if (((chose == SCAN_WDS) && (m_instance_wl[idx].wlCurrentChannel == channel)) ||
                    ...
                    if ( entry != NULL ) {
                        ...
                        if (strncmp(ssid, "\\x00", 4) == 0) {
                            strcpy (entry->ssid, ""); // 0 means empty ssid
                        } 
                        else {
                            strcpy(entry->ssid, ssid);
                        }
                        bcm_ether_ntoa(&bi->BSSID, entry->macAddress);
                        strcpy(entry->ifcName, m_instance_wl[idx].m_ifcName[MAIN_BSS_IDX]);
                        if (chose == SCAN_WPS) {
...
void wlmngr_scanWdsResult(int idx) {
   ...
   sprintf(wlwdsfile, "/var/wl%dwds", idx );

   fp = fopen(wlwdsfile, "w+");
   if ( fp != NULL ) {
      // active scan across all channels
      sprintf(cmd, "wlctl -i %s scan", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);
      ...
      // get scan result
      sprintf(cmd, "wlctl -i %s scanresults > /var/wl%dwds", m_instance_wl[idx].m_ifcName[0], idx ); 
      BCMWL_WLCTL_CMD(cmd);
   ...
   if ( fp != NULL ) {
      ...
      while ( fgets( cmd, sizeof(cmd), fp ) ) {
         switch(tagidx) {
            case 0:
               strcpy(tagstr, ID_SSID);
               ech = '"';
               break;
            case 1:
               strcpy(tagstr, ID_CHANNEL);
               ech = '\n';
               break;
            case 2:
               strcpy(tagstr, ID_BSSID);
               ech = '\t';
               break;
         }
...
void wlmngr_scanResult(int idx, int chose) {
   ...
   sprintf(wlscanfile, "/var/wl%dscan", idx );
   ...
   if ( fp != NULL ) {
      ...
      // get scan result
      sprintf(cmd, "wlctl -i %s scanresults > /var/wl%dscan", m_instance_wl[idx].m_ifcName[0], idx ); 
      BCMWL_WLCTL_CMD(cmd);
      sleep(1);
...
#undef CAP_STR_LEN
   ...
   sprintf(wlcapFile, "/var/wl%dcap", idx);
...
int wlmngr_getValidChannel(int idx, int channel) {
   ...
   sprintf(cmd, "wlctl -i wl%d channels > /var/wl%dchlist", idx, idx ); 
   BCMWL_WLCTL_CMD(cmd);
   sprintf(cmd, "/var/wl%dchlist", idx ); 
...
int wlmngr_getCoreRev(int idx) {
  ...
  sprintf(cmd, "wlctl -i wl%d revinfo > /var/wl%d", idx, idx);
  BCMWL_WLCTL_CMD(cmd);
  sprintf(cmd, "/var/wl%d",  idx);
...
char *wlmngr_getPhyType(int idx ) {
  ...
  sprintf(cmd, "wlctl -i wl%d phytype > /var/wl%d", idx,idx);
  BCMWL_WLCTL_CMD(cmd);
  sprintf(cmd, "/var/wl%d",  idx);
...
void wlmngr_setSsid(int idx) {
   ...
   for (i =0; i<m_instance_wl[idx].numBss; i++) {
         ...
         sprintf(cmd, "wlctl -i %s ssid -C %d %s", m_instance_wl[idx].m_ifcName[0], i, ssid);
         BCMWL_WLCTL_CMD(cmd);
   }
}
...
bool wlmngr_getWdsWsec(int idx, int unit, int which, char *mac, char *role, char *crypto, char *auth, ...)
{
   ...
   strcpy(value, m_instance_wl[idx].m_wlVar.wlWdsWsec);
   next = value;
...
#define ID_CURCHANNEL "target channel"
   ...
   if (!MIMO_PHY) {        
      sprintf(cmd, "wlctl -i wl%d channel > /var/curchannel%d", idx, idx);
      BCMWL_WLCTL_CMD(cmd);    
      sprintf(cmd, "/var/curchannel%d",  idx);
...
int wlmngr_getChannelImState(int idx) {
    ...
    sprintf(cmd, "wlctl -i wl%d chanim_state %d%s > /var/chanim_state", idx, m_instance_wl[idx].wlCurrentChannel, addi);
    BCMWL_WLCTL_CMD(cmd);

    sprintf(cmd, "/var/chanim_state");
    fp = fopen(cmd, "r");
...
#define ID_CURSPEC_SEP '('
   ...
   sprintf(cmd, "wlctl -i wl%d chanspec > /var/curchaspec%d", idx, idx );
   BCMWL_WLCTL_CMD(cmd);    
   sprintf(chanspecfile, "/var/curchaspec%d", idx );
...
void wlmngr_getHwAddr(int idx, char *ifname, char *hwaddr) {
   ...
   sprintf(cmd, "ifconfig %s > /var/hwaddr ", ifname);
   bcmSystemMute(cmd);
...
void wlmngr_setupMbssMacAddr(int idx)
{
   ...
   for (i=0; i<m_instance_wl[idx].numBss; i++) {
      sprintf(cmd, "ifconfig %s hw ether %s 2>/dev/null", m_instance_wl[idx].m_ifcName[i], m_instance_wl[idx].bssMacAddr[i]);
      bcmSystem(cmd);
#ifndef WLCONF
      sprintf(cmd, "wlctl -i %s cur_etheraddr %s  2>/dev/null", m_instance_wl[idx].m_ifcName[i], m_instance_wl[idx].bssMacAddr[i]);
      BCMWL_WLCTL_CMD(cmd);         
#endif
   } 
}
...
int wlmngr_getBands(int idx)
{
   ...
   sprintf(cmd, "wlctl -i wl%d bands > /var/wl%dbands", idx, idx ); 
   BCMWL_WLCTL_CMD(cmd);
   sprintf(cmd, "/var/wl%dbands", idx ); 
...
void wlmngr_getChannelList(int idx, int argc, char **argv, char *list)
{
   ...
   strcpy(curPhyType, m_instance_wl[idx].m_wlVar.wlPhyType); 
   ...
   sprintf(wlchspeclistfile, "/var/wl%dchspeclist", idx );
   ...
   *cmd = 0;
   if ((!strcmp(type, "b")) && (m_instance_wl[idx].m_bands & BAND_B)) {
         sprintf(cmd, "wlctl -i wl%d chanspecs -b %d -w %d -c %s > /var/wl%dchspeclist", \
...
void wlmngr_getNPhyRates(int idx, int argc, char **argv, char *list)
{
   ...
   /* Get configured phy type */
   strcpy(phyType, m_instance_wl[idx].m_wlVar.wlPhyType); 
   ...
   for (i = 1; i < (int)ARRAYSIZE(mcsidxs); i++) {
      ...
      if (mcsidxs[i] == -2) {
         ...
         if (m_instance_wl[idx].m_wlVar.wlNMcsidx == -2 &&  nbw == m_instance_wl[idx].m_wlVar.wlNBw) {
	   ...
	   curloc += sprintf(curloc, " or %d", mcs_rate/1000);	   
        ...
        curloc += sprintf(curloc, "\", \"%d\");\n", mcsidxs[i]);
      	...
      	if (m_instance_wl[idx].m_wlVar.wlNMcsidx == mcsidxs[i] && m_instance_wl[idx].m_wlVar.wlNBw == nbw)
           curloc += sprintf(curloc,"\t\tdocument.forms[0].wlNMmcsidx.selectedIndex ="
      		                 "%d;\n", i); 		                        
...
void wlmngr_aquireStationList(int idx) {
   ...
   int buflen= 0;
     ...
     sprintf(wl_authefile, "/var/wl%d_authe", idx);
     sprintf(wl_assocfile, "/var/wl%d_assoc", idx);
     sprintf(wl_authofile, "/var/wl%d_autho", idx);  
   ...
   for (i=0; i<m_instance_wl[idx].numBss && i < WL_MAX_NUM_SSID; i++) {
      ...
      sprintf(cmd, "wlctl -i %s authe_sta_list > /var/wl%d_authe", m_instance_wl[idx].m_ifcName[i], idx);
      BCMWL_WLCTL_CMD(cmd);
      sprintf(cmd, "wlctl -i %s assoclist > /var/wl%d_assoc", m_instance_wl[idx].m_ifcName[i], idx);
      BCMWL_WLCTL_CMD(cmd);
      sprintf(cmd, "wlctl -i %s autho_sta_list > /var/wl%d_autho", m_instance_wl[idx].m_ifcName[i], idx);  
      BCMWL_WLCTL_CMD(cmd);
               ...
               for (j=0; j<l; j++) {
                  if (fgets(buf, buflen, fp)) {                
                     if (wlmngr_scanForAddr(buf, strlen(buf), &pa, &asize)) {// find mac addr
                        ...
                        strcpy(curBssStaList[j].ssid, m_instance_wl[idx].m_wlMssidVar[i].wlSsid);
                        strcpy(curBssStaList[j].ifcName, m_instance_wl[idx].m_ifcName[i]);                        
                        //counters
...
static inline void wlmngr_togglePowerSave_aspm(int idx, char assoc, char *cmd)
{
    ...
    if (!strcmp(m_instance_wl[idx].m_wlVar.wlPhyType, WL_PHY_TYPE_AC))	{
        if (assoc)
            sprintf(cmd, "echo \"default\" > /sys/module/pcie_aspm/parameters/policy");
        else 
            /* Only enable L1 mode, because L0s creates EVM issues. The power savings are the same */
            sprintf(cmd, "echo \"l1_powersave\" > /sys/module/pcie_aspm/parameters/policy");
...
void wlmngr_togglePowerSave() {
   ...
   for (i=0; i<wl_cnt; i++) {
      if (associated[i]) {
          ...
          sprintf(cmd, "wlctl -i %s rxchain_pwrsave_enable 0", m_instance_wl[i].m_ifcName[0]);
          BCMWL_WLCTL_CMD(cmd);
      } else {
          /* restore back to the user setting */
          sprintf(cmd, "wlctl -i %s rxchain_pwrsave_enable %d", m_instance_wl[i].m_ifcName[0], m_instance_wl[i].m_wlVar.wlRxChainPwrSaveEnable);
          BCMWL_WLCTL_CMD(cmd);
      }
...
void wlmngr_getVer(int idx)
{
   ...
   sprintf(buf, "wlctl -i %s  ver > /var/wlver ", m_instance_wl[idx].m_ifcName[0]);
   BCMWL_WLCTL_CMD(buf);      
...
{  
   ...
   sprintf(cmd, "wlctl -i %s  antdiv %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlAntDiv);
   BCMWL_WLCTL_CMD(cmd);      
}
...
{  
   ...
   if (m_instance_wl[idx].m_wlVar.wlChannel == 0) {
      ...
      sprintf(cmd, "wlctl -i %s autochannel 1", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);
      sleep(1);
      sprintf(cmd, "wlctl -i %s autochannel 2", m_instance_wl[idx].m_ifcName[0]); 
      BCMWL_WLCTL_CMD(cmd);      
   ...
   sprintf(cmd, "wlctl -i %s csscantimer %d", m_instance_wl[idx].m_ifcName[0], timeout);
   BCMWL_WLCTL_CMD(cmd);             	
...
{  
   ...
   // reset
   sprintf(cmd, "wlctl -i %s regulatory 0", m_instance_wl[idx].m_ifcName[0]);  
   BCMWL_WLCTL_CMD(cmd);
  
   sprintf(cmd, "wlctl -i %s radar 0 2>/dev/null", m_instance_wl[idx].m_ifcName[0]); 
   BCMWL_WLCTL_CMD(cmd);
 
   sprintf(cmd, "wlctl -i %s spect 0 2>/dev/null", m_instance_wl[idx].m_ifcName[0]);
   BCMWL_WLCTL_CMD(cmd);
         
   sprintf(buf, "wl%d_reg_mode", idx);
   if ((m_instance_wl[idx].m_wlVar.wlRegMode == REG_MODE_H) && (m_instance_wl[idx].m_wlVar.wlBand == BAND_A)) {     

      nvram_set(buf, "h");
            
	  sprintf(cmd, "wlctl -i %s radar 1", m_instance_wl[idx].m_ifcName[0]); 
         BCMWL_WLCTL_CMD(cmd);
          
	  sprintf(cmd, "wlctl -i %s spect 1", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);           
         // set the CAC params		
         sprintf(cmd, "wlctl -i %s dfs_preism %d",  m_instance_wl[idx].m_ifcName[0],  m_instance_wl[idx].m_wlVar.wlDfsPreIsm);  
         BCMWL_WLCTL_CMD(cmd);
         sprintf(cmd, "wlctl -i %s dfs_postism %d",  m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlDfsPostIsm);  
         BCMWL_WLCTL_CMD(cmd);
         sprintf(cmd, "wlctl -i %s constraint %d",  m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlTpcDb);  
         BCMWL_WLCTL_CMD(cmd);                   
   } else if (m_instance_wl[idx].m_wlVar.wlRegMode == REG_MODE_D) {
      nvram_set(buf, "d");	
      sprintf(cmd, "wlctl -i %s regulatory 1", m_instance_wl[idx].m_ifcName[0]);  
      BCMWL_WLCTL_CMD(cmd);
   } else
      nvram_set(buf, "off");
...
void wlmngr_printSsidList(int idx, char *text)
{
   ...
   for (i=0; i<m_instance_wl[idx].numBss; i++) {
      sprintf(prtloc, "<option value='%d'>%s</option>\n%n", 
	  	i, m_instance_wl[idx].m_wlMssidVar[i].wlSsid, &wsize); prtloc +=wsize;
   }
   *prtloc = 0;
} 
...
void wlmngr_doQoS(int idx)
{
   ...
   if (m_instance_wl[idx].m_wlVar.wlWme) {
      for (i=0; i<m_instance_wl[idx].numBss; i++) {
         if (m_instance_wl[idx].m_wlMssidVar[i].wlEnblSsid && m_instance_wl[idx].m_wlVar.wlEnbl) {
            /* enable all the default queues associated with this ssid. */
            if ((ret = dalQos_setDefaultWlQueuesLock(m_instance_wl[idx].m_ifcName[i], TRUE)) != CMSRET_SUCCESS) {
            ...
            if ((ret = dalQos_setDefaultWlQueuesLock(m_instance_wl[idx].m_ifcName[i], FALSE)) != CMSRET_SUCCESS) {
               printf("dalQos_setDefaultWlQueuesLock() returns error. ret=%d\n", ret);
               return;
            }
         }
...
void wlmngr_doWlConf(int idx) {
   ...
   if (MIMO_PHY) {
      ...
      if(strcmp(m_instance_wl[idx].m_wlVar.wlNmode, WL_OFF)) {
         sprintf(name, "wlctl -i %s nreqd %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlNReqd);
...
void wlmngr_doBasic(int idx) {
   ...
   // set mode to AP
   sprintf(cmd, "wlctl -i %s  ap %d",m_instance_wl[idx].m_ifcName[0],  1);
   BCMWL_WLCTL_CMD(cmd);

   // set infra
   sprintf(cmd, "wlctl -i %s infra %d",m_instance_wl[idx].m_ifcName[0],  1);
   BCMWL_WLCTL_CMD(cmd);
   ...
   if(m_instance_wl[idx].mbssSupported) {   
      for (i=0; i<WL_NUM_SSID; i++) {
   	   if (m_instance_wl[idx].m_wlMssidVar[i].wlEnblSsid) {
            sprintf(cmd, "wlctl -i %s closed %d", m_instance_wl[idx].m_ifcName[i], m_instance_wl[idx].m_wlMssidVar[i].wlHide);
            BCMWL_WLCTL_CMD(cmd);
    	   }    		
      }
   } else {
      sprintf(cmd, "wlctl  -i  %s closed %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlMssidVar[MAIN_BSS_IDX].wlHide);
      BCMWL_WLCTL_CMD(cmd);
   }

   // country
   sprintf(cmd, "wlctl -i %s country %s",m_instance_wl[idx].m_ifcName[0],  m_instance_wl[idx].m_wlVar.wlCountry);
   BCMWL_WLCTL_CMD(cmd);
   //band
   if (m_instance_wl[idx].m_wlVar.wlBand == BAND_B) {
      sprintf(cmd, "wlctl -i %s  band b", m_instance_wl[idx].m_ifcName[0]);
   }
   else if (m_instance_wl[idx].m_wlVar.wlBand == BAND_A) {
      sprintf(cmd, "wlctl -i %s band a", m_instance_wl[idx].m_ifcName[0]);
   }
   else { // 0
      sprintf(cmd, "wlctl -i %s band auto", m_instance_wl[idx].m_ifcName[0]);
   }
   BCMWL_WLCTL_CMD(cmd);

   // regulatory
   wlmngr_setupRegulatory(idx);
      ...
      if(MIMO_PHY) {
         ...
         sprintf(cmd, "wlctl -i %s nmode %d", m_instance_wl[idx].m_ifcName[0], val);
         BCMWL_WLCTL_CMD(cmd);
      	
      	 m_instance_wl[idx].m_wlVar.wlNBwCap = (val != OFF) ? m_instance_wl[idx].m_wlVar.wlNBwCap : WL_N_BW_20ALL;
      
         //set bandwidth capability for nphy and calculate nbw
#ifdef WL11AC
         if (m_instance_wl[idx].m_wlVar.wlNBwCap == WL_V_BW_80IN5G)
            sprintf(cmd, "wlctl -i %s bw_cap 5g %d", m_instance_wl[idx].m_ifcName[0], WLC_BW_CAP_80MHZ);
         else
#endif
            sprintf(cmd, "wlctl -i %s mimo_bw_cap %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlNBwCap);

         BCMWL_WLCTL_CMD(cmd);

         if (((m_instance_wl[idx].m_wlVar.wlBand == BAND_B) && (m_instance_wl[idx].m_wlVar.wlNBwCap == WL_N_BW_40ALL)) ||
             ((m_instance_wl[idx].m_wlVar.wlBand == BAND_A) && (m_instance_wl[idx].m_wlVar.wlNBwCap == WL_N_BW_40ALL || m_instance_wl[idx].m_wlVar.wlNBwCap == WL_N_BW_20IN2G_40IN5G)))
         ...
         { /*
            ...
            */
            ...
            strcpy(tmp_s, nvram_safe_get(strcat_r(prefix, "nar", tmp)));

            if(*tmp_s)
                val = atoi(tmp_s);
            else 
                val = (m_instance_wl[idx].m_wlVar.wlBand == BAND_B) ? 0 : 0; /* 6_30_102_3 rel default is 1:0 */

            sprintf(cmd, "wlctl -i %s nar %d", m_instance_wl[idx].m_ifcName[0], val);
            BCMWL_WLCTL_CMD(cmd);
         }
      }
#endif      	
   ...
   if (m_instance_wl[idx].m_wlVar.wlChannel > 0) {
#ifdef SUPPORT_MIMO
      int channel;
      channel = m_instance_wl[idx].m_wlVar.wlChannel;
      ...
      if(MIMO_PHY) {         
         if (m_instance_wl[idx].m_wlVar.wlNBw == 80) {
            sprintf(cmd, "wlctl -i %s chanspec %d/%d",
            m_instance_wl[idx].m_ifcName[0], channel,  
            m_instance_wl[idx].m_wlVar.wlNBw);
         } else {
            sprintf(cmd, "wlctl -i %s chanspec -c %d -b %d -w %d -s %d",
            m_instance_wl[idx].m_ifcName[0], channel, 
            ((m_instance_wl[idx].m_wlVar.wlBand==BAND_B)? WL_CHANSPEC_2G: WL_CHANSPEC_5G), 
            m_instance_wl[idx].m_wlVar.wlNBw, m_instance_wl[idx].m_wlVar.wlNCtrlsb);
         }
      } else
#endif      
      sprintf(cmd, "wlctl -i %s channel %d",m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlChannel);
   ...
   // Set The WPA2 Pre auth cap
   for (i=0; i<m_instance_wl[idx].numBss; i++) {
      sprintf(cmd, "wlctl -i %s wpa_cap -C %d > /var/wpa_cap%d ",m_instance_wl[idx].m_ifcName[0], i, idx);
      BCMWL_WLCTL_CMD(cmd);
      val = 0;
      sprintf(cmd, "/var/wpa_cap%d ", idx);
      fp = fopen(cmd, "r");
      if ( fp != NULL ) {
         if (fgets(buf, sizeof(buf), fp)) {
         	val = atoi(buf);
      ...
      if(m_instance_wl[idx].m_wlMssidVar[i].wlPreauth)
      	val |= WPA_CAP_WPA2_PREAUTH;   	
      else
      	val &= ~WPA_CAP_WPA2_PREAUTH;
   
      sprintf(cmd, "wlctl -i %s wpa_cap -C %d %d 2>/dev/null",m_instance_wl[idx].m_ifcName[0],  i, val);
      BCMWL_WLCTL_CMD(cmd);
   }   
...
#include <fcntl.h>
#include <dirent.h>
static int prctl_getPidByName(const char *name)
{
   DIR *dir;
   FILE *fp;
   struct dirent *dent;
...
void wlmngr_doWlconfAddi(int idx) 
{
   ...
   if (MIMO_PHY) {   
      ...
      if ((chipinfo_stbc == 0) && (idx == 0)) {
          sprintf(cmd, "wlctl -i %s stbc_rx 0", m_instance_wl[idx].m_ifcName[0]);
          BCMWL_WLCTL_CMD(cmd);
      } else {
          sprintf(cmd, "wlctl -i %s stbc_rx %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlStbcRx);
          BCMWL_WLCTL_CMD(cmd);
      }

      /* Set Rxchain power save sta assoc check default to 1 */ 
      sprintf(cmd, "wlctl -i %s rxchain_pwrsave_stas_assoc_check %d", m_instance_wl[idx].m_ifcName[0], ON);
      BCMWL_WLCTL_CMD(cmd);

      /* Set Radio power save sta assoc check default to 1 */ 
      sprintf(cmd, "wlctl -i %s radio_pwrsave_stas_assoc_check %d", m_instance_wl[idx].m_ifcName[0], ON);
      BCMWL_WLCTL_CMD(cmd);
   }      
#endif    
   ...
   if(rateptr != rs_default) {
      sprintf(cmd,"wlctl -i %s rateset %s", m_instance_wl[idx].m_ifcName[0], rateptr);
      BCMWL_WLCTL_CMD(cmd);
   }
...
void wlmngr_doAdvanced(int idx) {
   ...
   // max association   
   if(m_instance_wl[idx].mbssSupported) {            	
      sprintf(cmd, "wlctl -i %s maxassoc %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlGlobalMaxAssoc);
      BCMWL_WLCTL_CMD(cmd);   
      ...
      for (i=0; i<m_instance_wl[idx].numBss; i++) {
         if(m_instance_wl[idx].m_wlMssidVar[i].wlMaxAssoc > m_instance_wl[idx].m_wlVar.wlGlobalMaxAssoc)
      	 ...
      	 if (m_instance_wl[idx].m_wlMssidVar[i].wlEnblSsid) {      		   
       	   sprintf(cmd, "wlctl -i %s bss_maxassoc %d", m_instance_wl[idx].m_ifcName[i], m_instance_wl[idx].m_wlMssidVar[i].wlMaxAssoc);
       	   BCMWL_WLCTL_CMD(cmd);
       	}  		
      }
   } else {
      m_instance_wl[idx].m_wlVar.wlGlobalMaxAssoc = m_instance_wl[idx].m_wlMssidVar[MAIN_BSS_IDX].wlMaxAssoc;
      sprintf(cmd, "wlctl -i %s maxassoc %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlMssidVar[MAIN_BSS_IDX].wlMaxAssoc);
      BCMWL_WLCTL_CMD(cmd);       
   }   
   ...
   if (m_instance_wl[idx].m_wlVar.wlBand == BAND_B) {
      ...
      switch ( m_instance_wl[idx].m_wlVar.wlgMode ) {
	     ...
	     sprintf(cmd, "wlctl -i %s gmode Performance", m_instance_wl[idx].m_ifcName[0]);
            break;
         case WL_MODE_G_LRS:
              sprintf(cmd, "wlctl -i %s gmode LRS", m_instance_wl[idx].m_ifcName[0]);
            break;
         case WL_MODE_B_ONLY:
              sprintf(cmd, "wlctl -i %s gmode legacy", m_instance_wl[idx].m_ifcName[0]);
            break;
         default:
              sprintf(cmd, "wlctl -i %s gmode Auto", m_instance_wl[idx].m_ifcName[0]);
            break;
      }
      ...
      // g protection
       ...
       sprintf(cmd, "wlctl -i %s gmode_protection_override %d", m_instance_wl[idx].m_ifcName[0], override);
       BCMWL_WLCTL_CMD(cmd);
       sprintf(cmd, "wlctl -i %s gmode_protection_control %d", m_instance_wl[idx].m_ifcName[0], control);
       BCMWL_WLCTL_CMD(cmd);       
   ...
   if (MIMO_PHY) {   
      ...
      if (!strcmp(m_instance_wl[idx].m_wlVar.wlNmode, WL_OFF)) {
         ...
         sprintf(cmd, "wlctl -i %s nmode_protection_override %d", m_instance_wl[idx].m_ifcName[0], override);
         BCMWL_WLCTL_CMD(cmd);
         sprintf(cmd, "wlctl -i %s protection_control %d", m_instance_wl[idx].m_ifcName[0], control);
         BCMWL_WLCTL_CMD(cmd);      	      	
      }

      if (m_instance_wl[idx].m_wlVar.wlRifsAdvert != 0)
         sprintf(cmd, "wlctl -i %s rifs_advert -1", m_instance_wl[idx].m_ifcName[0]);
      else
         sprintf(cmd, "wlctl -i %s rifs_advert 0", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);

      if ((m_instance_wl[idx].m_wlVar.wlNBw == 40) && (m_instance_wl[idx].m_wlVar.wlBand == BAND_B)) {
         sprintf(cmd, "wlctl -i %s obss_coex %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlObssCoex);
      } else {
        /* Need to disable obss coex in case of 20MHz and/or in case of 5G.  */
          sprintf(cmd, "wlctl -i %s obss_coex 0", m_instance_wl[idx].m_ifcName[0]);
      }
      BCMWL_WLCTL_CMD(cmd);
      ...
      sprintf(cmd, "wlctl -i %s stbc_tx %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlStbcTx);
      BCMWL_WLCTL_CMD(cmd);
   }
#endif
   ...
   if (MIMO_PHY) {
      // n required, i.e. if 1, supports only 11n STA, 11g STA cannot not assoc
      if(strcmp(m_instance_wl[idx].m_wlVar.wlNmode, WL_OFF)) {
         sprintf(cmd, "wlctl -i %s nreqd %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlNReqd);
         BCMWL_WLCTL_CMD(cmd);
      }     
   }      
#endif    
      ...
      val = OFF;       
      sprintf(cmd, "wlctl -i %s wme 0 2>/dev/null", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);      
      sprintf(cmd, "wlctl -i %s afterburner_override 0 2>/dev/null", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);      
      ...
      if(nmode) {
	    ...
	    sprintf(cmd, "wlctl -i %s ampdu 1", m_instance_wl[idx].m_ifcName[0]);
            BCMWL_WLCTL_CMD(cmd);          	
         }
         if(m_instance_wl[idx].amsduSupported) {         
            sprintf(cmd, "wlctl -i %s amsdu 1", m_instance_wl[idx].m_ifcName[0]);
            BCMWL_WLCTL_CMD(cmd);          	
         }
         if(m_instance_wl[idx].aburnSupported) {
            ...
            if (!strcmp(m_instance_wl[idx].m_wlVar.wlAfterBurnerEn, WL_OFF)) {
               sprintf(cmd, "wlctl -i %s afterburner_override 0", m_instance_wl[idx].m_ifcName[0]);
               BCMWL_WLCTL_CMD(cmd);
            } else {
               sprintf(cmd, "wlctl -i %s afterburner_override -1", m_instance_wl[idx].m_ifcName[0]);
               BCMWL_WLCTL_CMD(cmd);
            ...
            sprintf(cmd, "wlctl -i %s ampdu 0", m_instance_wl[idx].m_ifcName[0]);
            BCMWL_WLCTL_CMD(cmd);        
	    sprintf(cmd, "wlctl -i %s amsdu 0", m_instance_wl[idx].m_ifcName[0]);
            BCMWL_WLCTL_CMD(cmd);
            ...
            if(m_instance_wl[idx].aburnSupported) {
               ...
               if(m_instance_wl[idx].wmeSupported) {      
            	...
            	sprintf(cmd, "wlctl -i %s afterburner_override %d", m_instance_wl[idx].m_ifcName[0], val);
                BCMWL_WLCTL_CMD(cmd);            	
            }
        ...
        if (m_instance_wl[idx].wmeSupported) {
           if((m_instance_wl[idx].m_wlVar.wlWme!=OFF) && (val == OFF)) {
          	...
          	sprintf(cmd, "wlctl -i %s wme %d 2>/dev/null", m_instance_wl[idx].m_ifcName[0], val);
                BCMWL_WLCTL_CMD(cmd);
                sprintf(cmd, "wlctl -i %s wme_noack %d 2>/dev/null", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlWmeNoAck);
                BCMWL_WLCTL_CMD(cmd);
                sprintf(cmd, "wlctl -i %s wme_apsd %d 2>/dev/null", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlWmeApsd);
                BCMWL_WLCTL_CMD(cmd);
                ...
                if(m_instance_wl[idx].mbssSupported) {            	
                   for (i=0; i<m_instance_wl[idx].numBss; i++) {
      	              if (m_instance_wl[idx].m_wlMssidVar[i].wlEnblSsid) {
       	                 sprintf(cmd, "wlctl -i %s wme_bss_disable %d", m_instance_wl[idx].m_ifcName[i], m_instance_wl[idx].m_wlMssidVar[i].wlDisableWme);
       	                 BCMWL_WLCTL_CMD(cmd);
       	              }    		
                    }
                 }       
   ...
   if(m_instance_wl[idx].m_wlVar.wlBand == BAND_B) {
      if((MIMO_PHY)||
         ...
         if ( strcmp(m_instance_wl[idx].m_wlVar.wlPreambleType, "long") == 0 )
            sprintf(cmd, "wlctl -i %s plcphdr long", m_instance_wl[idx].m_ifcName[0]);
         else
            sprintf(cmd, "wlctl -i %s plcphdr auto", m_instance_wl[idx].m_ifcName[0]);   	   	
   ...
   if ( val  == 0 ) { /* auto rate */   
      sprintf(cmd, "wlctl -i %s rate %d", m_instance_wl[idx].m_ifcName[0], val);
      BCMWL_WLCTL_CMD(cmd);
      if(m_instance_wl[idx].m_bands & BAND_A) {
         sprintf(cmd, "wlctl -i %s a_rate %d", m_instance_wl[idx].m_ifcName[0], val);
         BCMWL_WLCTL_CMD(cmd);
      }
      if(m_instance_wl[idx].m_bands & BAND_B) {
         sprintf(cmd, "wlctl -i %s bg_rate %d ", m_instance_wl[idx].m_ifcName[0], val);
         BCMWL_WLCTL_CMD(cmd);
      }
   } else {/* manual rate */
      sprintf(cmd, "wlctl -i %s rate %3.1f ", m_instance_wl[idx].m_ifcName[0], (float)val / 1000000);
      BCMWL_WLCTL_CMD(cmd);
      if(m_instance_wl[idx].m_bands & BAND_A) {
         sprintf(cmd, "wlctl -i %s a_rate %3.1f ", m_instance_wl[idx].m_ifcName[0], (float)val / 1000000);
         BCMWL_WLCTL_CMD(cmd);
      }
      if(m_instance_wl[idx].m_bands & BAND_B) {
         sprintf(cmd, "wlctl -i %s bg_rate %3.1f ", m_instance_wl[idx].m_ifcName[0], (float)val / 1000000);
         BCMWL_WLCTL_CMD(cmd);
      }
   }      
   ...
   if (MIMO_PHY) {
      if(strcmp(m_instance_wl[idx].m_wlVar.wlNmode, WL_OFF)) {
         ...
         if (m_instance_wl[idx].m_wlVar.wlNMcsidx == 32 && m_instance_wl[idx].m_wlVar.wlNBw == 20) {         	
            m_instance_wl[idx].m_wlVar.wlNMcsidx = -1;
   	    ismcs = FALSE;
         }
         
         if(ismcs && (m_instance_wl[idx].m_wlVar.wlNMcsidx != 32)) {
            sprintf(cmd, "wlctl -i %s nrate -m %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlNMcsidx );
            BCMWL_WLCTL_CMD(cmd);         
         }          
      }     
   }
   ...
   if ( m_instance_wl[idx].m_wlVar.wlMCastRate == 0 ) {
      sprintf(cmd, "wlctl -i %s mrate -1 ", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);
      if(m_instance_wl[idx].m_bands & BAND_A) {
         sprintf(cmd, "wlctl -i %s a_mrate -1 ", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
      }
      if(m_instance_wl[idx].m_bands & BAND_B) {
         sprintf(cmd, "wlctl -i %s bg_mrate -1 ", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);     	
      }      
   } else {
      sprintf(cmd, "wlctl -i %s mrate %3.1f ", m_instance_wl[idx].m_ifcName[0], (float)m_instance_wl[idx].m_wlVar.wlMCastRate / 1000000);
      if(m_instance_wl[idx].m_bands & BAND_A) {
         sprintf(cmd, "wlctl -i %s a_mrate %3.1f ", m_instance_wl[idx].m_ifcName[0], (float)m_instance_wl[idx].m_wlVar.wlMCastRate / 1000000);
         BCMWL_WLCTL_CMD(cmd);
      }
      if(m_instance_wl[idx].m_bands & BAND_B) {
         sprintf(cmd, "wlctl -i %s bg_mrate %3.1f ", m_instance_wl[idx].m_ifcName[0], (float)m_instance_wl[idx].m_wlVar.wlMCastRate / 1000000);
         BCMWL_WLCTL_CMD(cmd);     	
      }       
   }
   ...
   // rts
   sprintf(cmd, "wlctl -i %s rtsthresh %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlRtsThrshld);
   BCMWL_WLCTL_CMD(cmd);
   // fragmentation
   sprintf(cmd, "wlctl -i %s fragthresh %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlFrgThrshld);
   BCMWL_WLCTL_CMD(cmd);
   // DTIM interval
   sprintf(cmd, "wlctl -i %s dtim %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlDtmIntvl);
   BCMWL_WLCTL_CMD(cmd);
   // beacon interval
   sprintf(cmd, "wlctl -i %s  bi %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlBcnIntvl);
   BCMWL_WLCTL_CMD(cmd);

   sprintf(cmd, "wlctl -i %s  bcn_rotate 1", m_instance_wl[idx].m_ifcName[0]);
   BCMWL_WLCTL_CMD(cmd);

   // framebursting mode
   if ( strcmp(m_instance_wl[idx].m_wlVar.wlFrameBurst, WL_OFF) == 0 )
      sprintf(cmd, "wlctl -i %s frameburst 0", m_instance_wl[idx].m_ifcName[0]);
   else
      sprintf(cmd, "wlctl -i %s frameburst 1", m_instance_wl[idx].m_ifcName[0]);
   BCMWL_WLCTL_CMD(cmd);
   ...
   if(m_instance_wl[idx].mbssSupported) {            	
      for (i=0; i<m_instance_wl[idx].numBss; i++) {
      	if (m_instance_wl[idx].m_wlMssidVar[i].wlEnblSsid) {
       		sprintf(cmd, "wlctl -i %s ap_isolate %d", m_instance_wl[idx].m_ifcName[i], m_instance_wl[idx].m_wlMssidVar[i].wlAPIsolation);
       		BCMWL_WLCTL_CMD(cmd);
       	}    		
      }
   } else {
      sprintf(cmd, "wlctl -i %s ap_isolate %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlMssidVar[MAIN_BSS_IDX].wlAPIsolation);
      BCMWL_WLCTL_CMD(cmd);
   }  
      

   // Tx Power Percentage
   sprintf(cmd, "wlctl -i %s pwr_percent %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlTxPwrPcnt);
   BCMWL_WLCTL_CMD(cmd);   
   ...
   if(m_instance_wl[idx].wmfSupported != 0) {
      for (i = 0; i < m_instance_wl[idx].numBss; i++) {
         sprintf(cmd, "wlctl -i %s wmf_bss_enable %d", m_instance_wl[idx].m_ifcName[i], m_instance_wl[idx].m_wlMssidVar[i].wlEnableWmf);
         BCMWL_WLCTL_CMD(cmd);
      }
   }
#endif
   ...
   if (m_instance_wl[idx].apstaSupported != 0) {
      sprintf(cmd, "wlctl -i %s apsta 0", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);
      sprintf(cmd, "wlctl -i %s ap 1", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);
      sprintf(cmd, "wlctl -i %s wet 0", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);
      sprintf(cmd, "wlctl -i %s mac_spoof 0", m_instance_wl[idx].m_ifcName[0]);
      BCMWL_WLCTL_CMD(cmd);
      sprintf(cmd, "%s_mode", m_instance_wl[idx].m_ifcName[0]);
      nvram_set(cmd, "ap"); 

      if (m_instance_wl[idx].m_wlVar.wlEnableUre) {

         sprintf(cmd, "wlctl -i %s apsta 1", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);

         if (m_instance_wl[idx].m_wlVar.wlEnableUre == 1) { /* bridge mode: Range Extender */
            sprintf(cmd, "wlctl -i %s wet 1", m_instance_wl[idx].m_ifcName[0]);
            BCMWL_WLCTL_CMD(cmd);
            sprintf(cmd, "%s_mode", m_instance_wl[idx].m_ifcName[0]); /* wl0 needs to be wet */
            nvram_set(cmd, "wet"); 
         ...
         for (i=1; i<m_instance_wl[idx].numBss; i++) { /* guest SSIDs need to be ap mode */
            sprintf(cmd, "%s_mode", m_instance_wl[idx].m_ifcName[i]);
            nvram_set(cmd, "ap"); 
         }
#ifdef SUPPORT_WSC
         ...
         if (!strcmp(m_instance_wl[idx].m_wlVar.wlMode, WL_OPMODE_WET) || !strcmp(m_instance_wl[idx].m_wlVar.wlMode, WL_OPMODE_WET_MACSPOOF)) {
             ...
             sprintf(cmd, "wlctl -i %s ap 0", m_instance_wl[idx].m_ifcName[0]);
             BCMWL_WLCTL_CMD(cmd);
             sprintf(cmd, "wlctl -i %s wet 1", m_instance_wl[idx].m_ifcName[0]);
             BCMWL_WLCTL_CMD(cmd);
             sprintf(cmd, "%s_mode", m_instance_wl[idx].m_ifcName[0]);
             ...
             {
                sprintf(cmd, "wlctl -i %s mac_spoof 1", m_instance_wl[idx].m_ifcName[0]);
                BCMWL_WLCTL_CMD(cmd);
             }
         }
      }
      sprintf(cmd, "wlctl -i %s sta_retry_time %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlStaRetryTime);
      BCMWL_WLCTL_CMD(cmd);
   } 
   ...
   if(m_instance_wl[idx].wlVecSupported != 0) {
         sprintf(cmd, "wlctl -i %s iperf %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].wlIperf);
         BCMWL_WLCTL_CMD(cmd);
         sprintf(cmd, "wlctl -i %s vec %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].wlVec);
         BCMWL_WLCTL_CMD(cmd);
   ...
   sprintf(cmd, "wlctl -i %s chanim_mode %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlChanImEnab);
   BCMWL_WLCTL_CMD(cmd);
   ...
   if (MIMO_PHY) {
      ...
      sprintf(cmd, "wlctl -i %s rxchain_pwrsave_stas_assoc_check %d", m_instance_wl[idx].m_ifcName[0], 1);
      BCMWL_WLCTL_CMD(cmd);

      /* Rxchain power save */
      sprintf(cmd, "wlctl -i %s rxchain_pwrsave_enable %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlRxChainPwrSaveEnable);
      BCMWL_WLCTL_CMD(cmd);
	  
	  sprintf(cmd, "wlctl -i %s rxchain_pwrsave_quiet_time %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlRxChainPwrSaveQuietTime);
      BCMWL_WLCTL_CMD(cmd);

      sprintf(cmd, "wlctl -i %s rxchain_pwrsave_pps %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlRxChainPwrSavePps);
      BCMWL_WLCTL_CMD(cmd);
      ...
      if ((strcmp(m_instance_wl[idx].m_wlVar.wlPhyType, WL_PHY_TYPE_N) == 0 ) || (strcmp(m_instance_wl[idx].m_wlVar.wlPhyType, WL_PHY_TYPE_AC) == 0 ))
      {
         ...
         {
            sprintf(cmd, "wl spect 1");
            BCMWL_WLCTL_CMD(cmd); 
         }
      }
	  
      sprintf(cmd, "wlctl -i %s radio_pwrsave_enable %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlRadioPwrSaveEnable);
      BCMWL_WLCTL_CMD(cmd);
	  
      sprintf(cmd, "wlctl -i %s radio_pwrsave_quiet_time %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlRadioPwrSaveQuietTime);
      BCMWL_WLCTL_CMD(cmd);

      sprintf(cmd, "wlctl -i %s radio_pwrsave_pps %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlRadioPwrSavePps);
      BCMWL_WLCTL_CMD(cmd);

      sprintf(cmd, "wlctl -i %s radio_pwrsave_level %d", m_instance_wl[idx].m_ifcName[0], m_instance_wl[idx].m_wlVar.wlRadioPwrSaveLevel);
      BCMWL_WLCTL_CMD(cmd);
   }
#endif
   ...
   if(idx == 1 && m_instance_wl[idx].wlMediaSupported != 0) {
         ...
         /* Enable frameburst */
         strcpy(m_instance_wl[idx].m_wlVar.wlFrameBurst, WL_ON); 
		 ...
		 /*
         sprintf(cmd, "wlctl -i %s wme_remap_tcp 1", m_instance_wl[idx].m_ifcName[0]);
         BCMWL_WLCTL_CMD(cmd);
		 */
   }
...
void wlmngr_write_nvram(void)
{
	...
	for (name = buf; *name && (strlen(str)<NVRAM_SPACE); name += strlen(name) + 1) {
		sprintf(pair, "%02X%s", strlen(name), name);
		strcat(str, pair);
#ifdef WL_WLMNGR_DBG   
		printf("%s", name);
#endif
...
void  wlmngr_genWscRandSssidPsk( int idx )
{
   ...
   sprintf(mode_str, "wl%s_wps_mode", wl_unit );
   sprintf(config_str, "wl%s_wps_config_state", wl_unit );
   sprintf(ir_str, "wl_wps_reg");

   if (unit == 0) {
         sprintf(config_str, "lan_wps_oob");
         sprintf(ir_str, "lan_wps_reg");
   }
   else {
         sprintf(config_str, "lan%s_wps_oob", wl_unit );
         sprintf(ir_str, "lan%s_wps_reg", wl_unit);
   }
    ...
    if ( !strcmp( wscmode, "enabled") &&  !strcmp(wscconfig,"0")  ) 
    {
		...
		/*generate Random PSK key and SSID*/			
		 if ( wlmngr_get_random_bytes((char *)&key_length, sizeof(key_length)) == WSC_SUCCESS) 
		 {
			...
			sprintf(dev_ssid, "%s%s", WL_DEFAULT_SSID, ssid_rand );
#ifdef WL_WLMNGR_DBG
			printf("Random ssid=%s\n", dev_ssid );
#endif		
			strcpy(m_instance_wl[unit].m_wlMssidVar[subunit].wlSsid, dev_ssid);
			sprintf(ssid_str, "wl%s_ssid", wl_unit );
			nvram_set(ssid_str, dev_ssid);
	   
			strcpy(m_instance_wl[unit].m_wlMssidVar[subunit].wlWpaPsk, random_key);
			sprintf(wpa_psk_str, "wl%s_wpa_psk", wl_unit );
			nvram_set(wpa_psk_str, random_key);
	
			strcpy(m_instance_wl[unit].m_wlMssidVar[subunit].wlAuthMode,"psk");
			sprintf(akm_str, "wl%s_akm", wl_unit );
