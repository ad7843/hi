    ...
    {      
        ...
        if(rutPMap_getAvailableInterfaceByKey(key, &IfStk, &pIf) == CMSRET_SUCCESS)
        {
            ...
            if (!cmsUtl_strcmp(pIf->interfaceType, MDMVS_LANINTERFACE))
            {
                ...
                sprintf(nv_name, "%s_ifname", lan_ifname);
	            ...
	            sprintf(nv_name, "%s_hwaddr", lan_ifname);
...
static int run_for_each_br(L2BridgingEntryObject *pBr, InstanceIdStack *pBrStk)
{
    ...
    if (pBr->bridgeKey == 0)
        sprintf(nv_name, "lan_ifnames");
    else
        sprintf(nv_name, "lan%d_ifnames", pBr->bridgeKey);
    ...
    if (pBr->bridgeKey == 0)
        sprintf(nv_name, "lan_ifname");
    else
        sprintf(nv_name, "lan%d_ifname", pBr->bridgeKey);

    sprintf(br, "br%d", pBr->bridgeKey);
    ...
    if (pBr->bridgeKey == 0)
        sprintf(nv_name, "lan_hwaddr");
    else
        sprintf(nv_name, "lan%d_hwaddr", pBr->bridgeKey);
