...
int wldsltr_alloc( int adapter_cnt ) 
{
   if ( (wl_dsl_tr = calloc( adapter_cnt, sizeof(struct wl_dsl_tr_struct) )) == NULL ) {
   	printf("%s@%d alloc wl_dsl_tr[size %d] Err \n", __FUNCTION__, __LINE__, 
   ...
   {
       printf("%s@%d alloc wl_dsl_tr_presharedkey_struct[size %d] Err \n", __FUNCTION__, __LINE__, 
...
/* status is dynamic */
   ...
   strcpy( wl_dsl_trPtr->BSSID, wl_instance->bssMacAddr[MAIN_BSS_IDX] );
   ...
   strcpy( wl_dsl_trPtr->SSID, m_wlMssidVarPtr->wlSsid );

    wl_dsl_tr_get_brcm_beaconType(m_wlMssidVarPtr->wlAuthMode, 
                                                           wl_dsl_trPtr->beaconType );
	

   if ( !strcmp( m_wlMssidVarPtr->wlFltMacMode, WL_FLT_MAC_ALLOW) )
   ...
   strcpy(wl_dsl_trPtr->keyPassphrase, "NOS"); /*Not Support*/

   switch ( m_wlMssidVarPtr->wlKeyBit ) {
   	case WL_BIT_KEY_64:
             strcpy( wl_dsl_trPtr->WEPEncryptionLevel, "40-bit");
	      break;
       case   WL_BIT_KEY_128:
     	      strcpy( wl_dsl_trPtr->WEPEncryptionLevel, "104-bit");
	      break;
	default:
  	      strcpy( wl_dsl_trPtr->WEPEncryptionLevel, "Disabled");
		break;
   }
...
*/
   ...
   if ( !strcmp(m_wlMssidVarPtr->wlWep, WL_DISABLED) )
   {
      strcpy(wl_dsl_trPtr->basicEncryptionModes, "None");
   }
   else
   {
      strcpy(wl_dsl_trPtr->basicEncryptionModes, "WEPEncryption");
   }

   /*basicAuthenticationMode*/
   if ( !strcmp(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_RADIUS) )
   {
      strcpy(wl_dsl_trPtr->basicAuthenticationMode, "EAPAuthentication");    
   }
   else if ( !strcmp(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_SHARED) )
   {
      strcpy(wl_dsl_trPtr->basicAuthenticationMode, "SharedAuthentication");    
   }
   else
   {
      strcpy(wl_dsl_trPtr->basicAuthenticationMode, "None");    
   }

   /* WPAAuthenticationMode*/
   if(strcmp(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA_PSK)==0)
   {
      strcpy(wl_dsl_trPtr->WPAAuthenticationMode, "PSKAuthentication");    
   }
   else if(strcmp(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA)==0)
   {
      strcpy(wl_dsl_trPtr->WPAAuthenticationMode, "EAPAuthentication");    
   }
   else if(strcmp(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA2_PSK)==0)
   {
      strcpy(wl_dsl_trPtr->IEEE11iAuthenticationMode, "PSKAuthentication");    
   }
   else if(strcmp(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA2)==0)
   {
      strcpy(wl_dsl_trPtr->IEEE11iAuthenticationMode, "EAPAuthentication");    
   }
   else
   {
      strcpy(wl_dsl_trPtr->WPAAuthenticationMode, "PSKAuthentication");    
   ...
   /* WPAEncryptionModes */
   if(strcmp(m_wlMssidVarPtr->wlWep, WL_ENABLED)==0)
   {
      if(strcmp(m_wlMssidVarPtr->wlWpa, TKIP_ONLY)==0)
      {
         strcpy(wl_dsl_trPtr->WPAEncryptionModes, "WEPandTKIPEncryption");    
         strcpy(wl_dsl_trPtr->IEEE11iEncryptionModes, "WEPandTKIPEncryption"); 
      }
      else if(strcmp(m_wlMssidVarPtr->wlWpa, AES_ONLY)==0)
      {
         strcpy(wl_dsl_trPtr->WPAEncryptionModes, "WEPandAESEncryption");    
         strcpy(wl_dsl_trPtr->IEEE11iEncryptionModes, "WEPandAESEncryption"); 
      }
      else if(strcmp(m_wlMssidVarPtr->wlWpa, TKIP_AND_AES)==0)
      {
         strcpy(wl_dsl_trPtr->WPAEncryptionModes, "WEPandTKIPandAESEncryption");    
         strcpy(wl_dsl_trPtr->IEEE11iEncryptionModes, "WEPandTKIPandAESEncryption"); 
      }
      else
      {
         strcpy(wl_dsl_trPtr->WPAEncryptionModes, "WEPEncryption");    
         strcpy(wl_dsl_trPtr->IEEE11iEncryptionModes, "WEPEncryption"); 
      }
   }
   else
   {
      if(strcmp(m_wlMssidVarPtr->wlWpa, TKIP_ONLY)==0)
      {
         strcpy(wl_dsl_trPtr->WPAEncryptionModes, "TKIPEncryption");    
         strcpy(wl_dsl_trPtr->IEEE11iEncryptionModes, "TKIPEncryption");
      }
      else if(strcmp(m_wlMssidVarPtr->wlWpa, AES_ONLY)==0)
      {
         strcpy(wl_dsl_trPtr->WPAEncryptionModes, "AESEncryption");   
         strcpy(wl_dsl_trPtr->IEEE11iEncryptionModes, "AESEncryption");
      }
      else if(strcmp(m_wlMssidVarPtr->wlWpa, TKIP_AND_AES)==0)
      {
         strcpy(wl_dsl_trPtr->WPAEncryptionModes, "TKIPandAESEncryption");   
         strcpy(wl_dsl_trPtr->IEEE11iEncryptionModes, "TKIPandAESEncryption"); 
      }
   }
   ...
   strcpy( wl_dsl_trPtr->operationalDataTransmitRates, "NOS");	/**No Support  */
   strcpy( wl_dsl_trPtr->possibleDataTransmitRates, "NOS" );	       /** No Support */
...
/* radioEnabled is Dynamic */
    ...
    strcpy( wl_dsl_trPtr->locationDescription, "NOS");	/**No Support*/
    ...
    wl_dsl_trPtr->regulatoryDomain[2] = 0;
     ...
     strcpy(wl_dsl_trPtr->channelsInUse, "NOS");	/** No Support*/


     if ( !strcmp(m_wlVarPtr->wlMode, WL_OPMODE_AP ) ) 
         strcpy( wl_dsl_trPtr->deviceOperationMode, "InfrastructureAccessPoint");
     if ( !strcmp(m_wlVarPtr->wlMode, WL_OPMODE_WDS) ) 
         strcpy( wl_dsl_trPtr->deviceOperationMode, "WireleseeBridge");
	 	
//  strcpy( wl_dsl_trPtr->deviceOperationMode, "NOS");	/** No Support*/
	 
     wl_dsl_trPtr->distanceFromRoot =0;	/**No Support*/
     strcpy( wl_dsl_trPtr->peerBSSID, "NOS");	/** */
     strcpy( wl_dsl_trPtr->authenticationServiceMode, "NOS");	/**No Support */
...
/*Setup Wepkey*/
    for (i = 0; i < 4; i++) {
    	if ( m_wlMssidVarPtr->wlKeyBit == WL_BIT_KEY_64 )
    	    strcpy( wl_dsl_tr_wepkeyPtr->WEPKey[i], m_wlMssidVarPtr->wlKeys64[i]);
    	else
    	    strcpy( wl_dsl_tr_wepkeyPtr->WEPKey[i], m_wlMssidVarPtr->wlKeys128[i]);  
    }     

    for (i = 0; i < 1; i++) 
    {
        strcpy( wl_dsl_tr_presharedkeyPtr->PreSharedKey[i], m_wlMssidVarPtr->wlWpaPsk);
    }     
...
*/
    ...
    strcpy(wl_dsl_trPtr->X_BROADCOM_COM_IfName,  wl_instance->m_ifcName[0]);
    ...
    for (i=0; i<WL_NUM_SSID-1; i++) {
        ...
        strcpy(wl_dsl_tr_mbssPtr->GuestSSID,  m_wlMssidVarPtr->wlSsid);
        strcpy( wl_dsl_tr_mbssPtr->GuestBSSID, wl_instance->bssMacAddr[i+GUEST_BSS_IDX] );
...
/* BSSID is ReadOnly dynamic */
   ...
   strcpy( m_wlMssidVarPtr->wlSsid, wl_dsl_trPtr->SSID );

/*Set beaconType, next Security Part*/

   if ( wl_dsl_trPtr->MACAddressControlEnabled != 0 )
        strcpy(m_wlMssidVarPtr->wlFltMacMode, WL_FLT_MAC_ALLOW);
    else
        strcpy(m_wlMssidVarPtr->wlFltMacMode, WL_FLT_MAC_DENY);
...
/*Standard is readOnly*/  
   ...
   if ( !strcmp( wl_dsl_trPtr->WEPEncryptionLevel, "40-bit") )
      m_wlMssidVarPtr->wlKeyBit  =WL_BIT_KEY_64;
   if ( !strcmp( wl_dsl_trPtr->WEPEncryptionLevel, "104-bit") )
      m_wlMssidVarPtr->wlKeyBit  =WL_BIT_KEY_128;
   if ( !strcmp( wl_dsl_trPtr->WEPEncryptionLevel, "Disabled") )
      m_wlMssidVarPtr->wlKeyBit  =WL_BIT_KEY_128;
...
/* NOS
    ...
    locationDescription
*/
    strcpy(m_wlVarPtr->wlCountry, wl_dsl_trPtr->regulatoryDomain);
...
*/
     ...
     if ( !strcmp( wl_dsl_trPtr->deviceOperationMode, "InfrastructureAccessPoint") )
         strcpy(m_wlVarPtr->wlMode, WL_OPMODE_AP );
     if ( !strcmp(wl_dsl_trPtr->deviceOperationMode, "WireleseeBridge") )
         strcpy(m_wlVarPtr->wlMode, WL_OPMODE_WDS );
...
/*Setup Wepkey*/
    for (i = 0; i < 4; i++) {
    	if ( m_wlMssidVarPtr->wlKeyBit == WL_BIT_KEY_64 )
    	    strcpy( m_wlMssidVarPtr->wlKeys64[i], wl_dsl_tr_wepkeyPtr->WEPKey[i] );
    	else
    	    strcpy( m_wlMssidVarPtr->wlKeys128[i], wl_dsl_tr_wepkeyPtr->WEPKey[i]);  
    }     

/*Setup PreSharedKey*/
    for (i = 0; i < 1; i++)
    { 
       strcpy( m_wlMssidVarPtr->wlWpaPsk, wl_dsl_tr_presharedkeyPtr->PreSharedKey[i] );    
    }
...
/*Beacon Type*/
    if ( strcmp (wl_dsl_trPtr->beaconType, "Basic") == 0 ) {
                    brcm_BeaconType = Wlan_BeaconType_Basic;
    } else if ( strcmp(wl_dsl_trPtr->beaconType, "WPA") == 0 ) {
                    brcm_BeaconType = Wlan_BeaconType_WPA;
    } else if ( strcmp(wl_dsl_trPtr->beaconType, "11i") == 0 ) {
                    brcm_BeaconType = Wlan_BeaconType_11i;
    } else if ( strcmp(wl_dsl_trPtr->beaconType, "BasicandWPA") == 0 ) {
                    brcm_BeaconType = Wlan_BeaconType_BasicandWPA;
    } else if ( strcmp(wl_dsl_trPtr->beaconType, "Basicand11i") == 0 ) {
                    brcm_BeaconType = Wlan_BeaconType_Basicand11i;
    } else if ( strcmp(wl_dsl_trPtr->beaconType, "WPAand11i") == 0 ) {
                    brcm_BeaconType = Wlan_BeaconType_WPAand11i;
    } else if ( strcmp(wl_dsl_trPtr->beaconType, "BasicandWPAand11i") == 0 ) {
...
char *    IEEE11iAuthenticationMode;
*******************************/
    if ( strcmp(wl_dsl_trPtr->basicEncryptionModes, "WEPEncryption") == 0 )
         brcm_BasicEncryptionModes = Wlan_BasicEncryptionModes_WEPEncryption;
    else if ( strcmp(wl_dsl_trPtr->basicEncryptionModes, "None") == 0 )
         brcm_BasicEncryptionModes = Wlan_BasicEncryptionModes_None;

    if ( strcmp(wl_dsl_trPtr->basicAuthenticationMode, "None") == 0 ) {
                brcm_BasicAuthenticationMode = Wlan_BasicAuthenticationMode_None;
    } else if ( strcmp(wl_dsl_trPtr->basicAuthenticationMode, "EAPAuthentication") == 0 )  {
                brcm_BasicAuthenticationMode = Wlan_BasicAuthenticationMode_EAPAuthentication;
    } else if ( strcmp(wl_dsl_trPtr->basicAuthenticationMode, "SharedAuthentication") == 0 )  {
                brcm_BasicAuthenticationMode = Wlan_BasicAuthenticationMode_SharedAuthentication;
    }


    if ( strcmp(wl_dsl_trPtr->WPAEncryptionModes, "WEPEncryption") == 0 ) {
                brcm_WPAEncryptionModes =Wlan_WPAEncryptionModes_WEPEncryption;
    } else if ( strcmp(wl_dsl_trPtr->WPAEncryptionModes, "TKIPEncryption") == 0 ) {
                brcm_WPAEncryptionModes =Wlan_WPAEncryptionModes_TKIPEncryption;
    } else if ( strcmp(wl_dsl_trPtr->WPAEncryptionModes, "WEPandTKIPEncryption") == 0 ) {
                brcm_WPAEncryptionModes = Wlan_WPAEncryptionModes_WEPandTKIPEncryption;
    } else if ( strcmp(wl_dsl_trPtr->WPAEncryptionModes, "AESEncryption") == 0 ) {
                brcm_WPAEncryptionModes = Wlan_WPAEncryptionModes_AESEncryption;
    } else if ( strcmp(wl_dsl_trPtr->WPAEncryptionModes, "WEPandAESEncryption") == 0 ) {
                brcm_WPAEncryptionModes = Wlan_WPAEncryptionModes_WEPandAESEncryption;
    } else if ( strcmp(wl_dsl_trPtr->WPAEncryptionModes, "TKIPandAESEncryption") == 0 ) {
                brcm_WPAEncryptionModes = Wlan_WPAEncryptionModes_TKIPandAESEncryption;
    } else if ( strcmp(wl_dsl_trPtr->WPAEncryptionModes, "WEPandTKIPandAESEncryption") == 0 ) {
                brcm_WPAEncryptionModes = Wlan_WPAEncryptionModes_WEPandTKIPandAESEncryption;
    }
	 
    if ( strcmp(wl_dsl_trPtr->WPAAuthenticationMode, "PSKAuthentication") == 0 ) {
            brcm_WPAAuthenticationMode = Wlan_WPAAuthenticationMode_PSKAuthentication;
    } else if ( strcmp(wl_dsl_trPtr->WPAAuthenticationMode, "EAPAuthentication") == 0 ) {
                brcm_WPAAuthenticationMode = Wlan_WPAAuthenticationMode_EAPAuthentication;
    }

    if ( strcmp(wl_dsl_trPtr->IEEE11iEncryptionModes, "WEPEncryption") == 0 ) {
            brcm_IEEE11iEncryptionModes = Wlan_IEEE11iEncryptionModes_WEPEncryption;
    } else  if ( strcmp(wl_dsl_trPtr->IEEE11iEncryptionModes, "TKIPEncryption") == 0 ) {
            brcm_IEEE11iEncryptionModes = Wlan_IEEE11iEncryptionModes_TKIPEncryption;
    } else  if ( strcmp(wl_dsl_trPtr->IEEE11iEncryptionModes, "WEPandTKIPEncryption") == 0 ) {
            brcm_IEEE11iEncryptionModes = Wlan_IEEE11iEncryptionModes_WEPandTKIPEncryption;
    } else  if ( strcmp(wl_dsl_trPtr->IEEE11iEncryptionModes, "AESEncryption") == 0 ) {
            brcm_IEEE11iEncryptionModes = Wlan_IEEE11iEncryptionModes_AESEncryption;
    } else  if ( strcmp(wl_dsl_trPtr->IEEE11iEncryptionModes, "WEPandAESEncryption") == 0 ) {
            brcm_IEEE11iEncryptionModes = Wlan_IEEE11iEncryptionModes_WEPandAESEncryption;
    } else  if ( strcmp(wl_dsl_trPtr->IEEE11iEncryptionModes, "TKIPandAESEncryption") == 0 ) {
            brcm_IEEE11iEncryptionModes = Wlan_IEEE11iEncryptionModes_TKIPandAESEncryption;
    } else  if ( strcmp(wl_dsl_trPtr->IEEE11iEncryptionModes, "WEPandTKIPandAESEncryption") == 0 ) {
            brcm_IEEE11iEncryptionModes = Wlan_IEEE11iEncryptionModes_WEPandTKIPandAESEncryption;
    }
         
    if ( strcmp(wl_dsl_trPtr->IEEE11iAuthenticationMode, "PSKAuthentication") == 0 ) {
            brcm_IEEE11iAuthenticationMode = Wlan_IEEE11iAuthenticationMode_PSKAuthentication;
    } else if ( strcmp(wl_dsl_trPtr->IEEE11iAuthenticationMode, "EAPAuthentication") == 0 ) {
                brcm_IEEE11iAuthenticationMode = Wlan_IEEE11iAuthenticationMode_EAPAuthentication;
    } else if ( strcmp(wl_dsl_trPtr->IEEE11iAuthenticationMode, "EAPandPSKAuthentication") == 0 ) {
...
/*Setup Security params*/
    ...
    switch ( wepEnabled ) {
		case WepDisabled:
                   strcpy(m_wlMssidVarPtr->wlWep, WL_DISABLED);
		     break;
              case WepEnabled:
                   strcpy(m_wlMssidVarPtr->wlWep, WL_ENABLED);
		     break;
    }             
    ...
    if( authMode == WlanAuthMode_Radius )
        strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_RADIUS);
                  
    //WpaAkm
    if( WpaAkm.u.bitVal.Wlan_WpaAKm_Unspecified && WpaAkm.u.bitVal.Wlan_Wpa2AKm_Unspecified ) {
        // WPA and WPA2 
        strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA2_MIX);
    } else if( WpaAkm.u.bitVal.Wlan_WpaAKm_Unspecified ) {
        // WPA
        strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA);
    } else if( WpaAkm.u.bitVal.Wlan_Wpa2AKm_Unspecified ) {
        // WPA2 
        strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA2);
    } else if (WpaAkm.u.bitVal.Wlan_WpaAKm_Psk && WpaAkm.u.bitVal.Wlan_Wpa2AKm_Psk ) {  	
        // WPA-PSK and WPA2-PSK
        strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA2_PSK_MIX);  
    } else if (WpaAkm.u.bitVal.Wlan_WpaAKm_Psk ) {
        // WPA-PSK
        strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA_PSK);
    } else if (WpaAkm.u.bitVal.Wlan_Wpa2AKm_Psk ) {
        // WPA2-PSK        
        strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_WPA2_PSK);
    } else
        //none and open
        strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_OPEN);    
    ...
    if( wepEnabled == WepEnabled)
    {
        ...
        {
            strcpy(m_wlMssidVarPtr->wlAuthMode, WL_AUTH_SHARED);
        }
    }    
    ...
    switch ( crypto ) {
        case Wlan_Crypto_Aes:
             strcpy(m_wlMssidVarPtr->wlWpa, AES_ONLY);
             break;        
        case Wlan_Crypto_Tkip_Aes:                
             strcpy(m_wlMssidVarPtr->wlWpa, TKIP_AND_AES);
             break;
        case Wlan_Crypto_Tkip:
        default:
             strcpy(m_wlMssidVarPtr->wlWpa, TKIP_ONLY);
             break;              
    }
...
//RO    strcpy(wl_instance->m_ifcName[0], wl_dsl_trPtr->X_BROADCOM_COM_IfName);
    m_wlMssidVarPtr =  &(wl_instance->m_wlMssidVar[MAIN_BSS_IDX]);
    ...
    for (i=0; i<WL_NUM_SSID-1; i++) {
        ...
        strcpy( m_wlMssidVarPtr->wlSsid, wl_dsl_tr_mbssPtr->GuestSSID);
...
void wl_dsl_tr_get_brcm_beaconType( char *wlAuthMode, char *value )
{
    ...
    if (  !strcmp(wlAuthMode, WL_AUTH_OPEN) || 
	    !strcmp(wlAuthMode, WL_AUTH_SHARED) ||  
	    !strcmp(wlAuthMode, WL_AUTH_RADIUS)  )
		strcpy(value, "Basic");

   if ( !strcmp(wlAuthMode, WL_AUTH_WPA) || 
   	  !strcmp(wlAuthMode, WL_AUTH_WPA_PSK) ) 
		strcpy(value, "WPA");

    if ( !strcmp(wlAuthMode, WL_AUTH_WPA2) || 
	  !strcmp(wlAuthMode, WL_AUTH_WPA2_PSK) ) 
		strcpy(value, "11i");

	if ( !strcmp(wlAuthMode, WL_AUTH_WPA2_MIX) ||
	  !strcmp(wlAuthMode, WL_AUTH_WPA2_PSK_MIX) ) 
...
void wl_dsl_tr_get_maxBitRate(int wlBand, long wlRate, int wlgMode, char *value)
{
       switch ( (Wlan_TxRate)wlRate ) {
		case Wlan_TxRate_Auto:
                    switch (wlBand ) {
                          case BAND_A:
                                strcpy(value, "54"); // 802.11a
                                break;
                          case BAND_B:
                                 if ( wlgMode == 0 )
                                      strcpy(value , "11" ); // 802.11b
                                 else 
                                       strcpy(value, "54");
                                  break;
                   }
...
void wl_dsl_tr_set_maxBitRate(char *maxBitRate, int wlBand, int wlgMode, long *wlRate)
{
       if ( strcmp(maxBitRate, "Auto") == 0 ) {
                ...
                } else if ( (wlBand == BAND_B) || (wlgMode == 0) ) {  // 802.11b only
...
void wl_dsl_tr_get_basicDataTransmitRates(int wlBand, int wlgMode, char *wlBasicRate, char *value)
{
     switch ( wlBand) {
	  case BAND_A:
               if ( !strcmp(wlBasicRate, WL_BASIC_RATE_DEFAULT) )
                      strcpy(value, "54" );
               if ( !strcmp(wlBasicRate, WL_BASIC_RATE_ALL) )
                      strcpy(value, "1,2,5.5,6,9,11,12,18,24,36,48,54");
               if ( !strcmp(wlBasicRate, WL_BASIC_RATE_WIFI_2) )
                      strcpy(value, "6,12,24");
               if ( !strcmp(wlBasicRate, WL_BASIC_RATE_1_2) )
                       strcpy(value, "6,12");
               break;
         case BAND_B:
	   	  if ( wlgMode == 0 ) {
...
void wl_dsl_tr_set_basicDataTransmitRates(char *basicDataTransmitRates, int wlBand, int wlgMode, char *wlBasicRate)
{
            ...
            if ( wlBand == BAND_A ) {
                ...
                } else if ( strcmp(ptr, "1,2,5.5,6,9,11,12,18,24,36,48,54") == 0 ) {
                    strcpy(wlBasicRate, WL_BASIC_RATE_ALL);
                } else if ( strcmp(ptr, "6,12,24") == 0 ) {
                    strcpy(wlBasicRate,  WL_BASIC_RATE_WIFI_2);
                } else if ( strcmp(ptr, "6,12") == 0 ) {
                    strcpy(wlBasicRate, WL_BASIC_RATE_1_2);
                }
            } else if ( wlgMode == 0) { //b mode rate????
                if ( strcmp(ptr, "11") == 0 ) {
                    strcpy(wlBasicRate, WL_BASIC_RATE_DEFAULT);
                } else if ( strcmp(ptr, "1,2,5.5,6,9,11") == 0 ) {
                    strcpy(wlBasicRate, WL_BASIC_RATE_ALL);
                } else if ( strcmp(ptr, "1,2,5.5,6,11,12,24") == 0 ) {
                    strcpy(wlBasicRate,  WL_BASIC_RATE_WIFI_2);
                } else if ( strcmp(ptr, "1,2") == 0 ) {
                    strcpy(wlBasicRate, WL_BASIC_RATE_1_2);
                }
            } else { //G Mode
                if ( strcmp(ptr, "54") == 0 ) {
