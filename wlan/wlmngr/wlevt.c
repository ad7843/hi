...
void wlevt_send_msg(int idx)
{
	...
	sprintf((char *)(msg + 1), "Assoc:%d", idx+1);
...
/*Send event to ssk to update WAN side link status*/
#ifdef DMP_X_BROADCOM_COM_WIFIWAN_1
void wlevt_send_msg_to_ssk(void)
{
   static char buf[sizeof(CmsMsgHeader) + 32]={0};
   CmsRet ret = CMSRET_INTERNAL_ERROR;
   ...
   msg->dataLength = 32;
   msg->type = CMS_MSG_WLAN_LINK_STATUS_CHANGED;
   msg->src = EID_WLEVENT;
   msg->dst = EID_SSK;
   msg->flags_event = 1;
   msg->flags_request = 0;
   ...
   if ((ret = cmsMsg_send(msgHandle, msg)) != CMSRET_SUCCESS)
      printf("could not send CMS_MSG_WLAN_LINK_STATUS_CHANGED msg to wlmngr, ret=%d", ret);
#ifdef DSLCPE_EVT
   else
      printf("message CMS_MSG_WLAN_LINK_STATUS_CHANGED sent successfully");
#endif
	...
	while ( 1 ) {
		...
		/* Parsing event here. WLC_E_xxx event definition is in bcmevent.h */
			...
			printf("version=%x flags=%x event_type=%x status=%x reason=%x auth_type=%x datalen=%x\n",
				dpkt->event.version, dpkt->event.flags, dpkt->event.event_type, dpkt->event.status, dpkt->event.reason, 
			...
			*/
			...
			if ((dpkt->event.event_type == WLC_E_AUTH_IND) ||
				(dpkt->event.event_type == WLC_E_DEAUTH_IND) ||
				(dpkt->event.event_type == WLC_E_ASSOC_IND) ||
				(dpkt->event.event_type == WLC_E_REASSOC_IND) ||
				(dpkt->event.event_type == WLC_E_DISASSOC_IND)) {
...
#ifdef DMP_X_BROADCOM_COM_WIFIWAN_1
            /* Event WLC_E_LINK is passed to ssk to update WAN link status*/
            if (dpkt->event.event_type == WLC_E_LINK) {
               /* Send event to update Wanwifi status*/
               wlevt_send_msg_to_ssk();
            }	
#endif
